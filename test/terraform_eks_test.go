...
func TestTerraformEksJX(t *testing.T) {
	...
	iamClient := aws2.NewIamClient(t, region)
	...
	kmsClient := aws2.NewKmsClient(t, region)
	_, err = kmsClient.DescribeKey(&kms.DescribeKeyInput{KeyId: aws.String(vaultKMS)})
	assert.NoError(t, err)
}
